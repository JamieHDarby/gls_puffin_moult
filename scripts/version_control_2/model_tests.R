...
library(caret)
  ...
  pr <- as.numeric(predict(mod, mod_df, type="response", na.rm = T))            
  
  # the final model is used to predict the data on the response scale (i.e. a value between 0 and 1)
  pred <- prediction(pr[which(!is.na(pr))], mod_df$bin_moult[which(!is.na(pr))])
  ...
  DATA$Observed <- ifelse(mod_df$bin_moult == 0, 0, 1)
