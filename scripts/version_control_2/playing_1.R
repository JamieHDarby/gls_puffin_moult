...
date.range.plot <- 
  date_range %>%
  mutate(area = ifelse(area == "Norwegian Sea", "Norwegian Sea (n = 26)", area),
         area = ifelse(area == "Iceland", "Iceland (n = 12)", area),
         area = ifelse(area == "Barents Sea", "Barents Sea (n = 15)", area),
         area = ifelse(area == "UK", "UK (n = 6)", area)) %>%
  ggplot() +
...
atlantic_plot <-
  ...
  coord_map(projection = "azequidistant",
            xlim = c(-70, 0),
            ylim = c(35, 60)) +
  geom_point(aes(x = lon, y = lat, colour = colony), size = 8, alpha = 0.7) +
...
iceland_plot <-
  ggplot(meta_df %>% filter(area %in% c("Iceland"))) +
  coord_map(projection = "azequidistant",
            xlim = c(-60, -15),
            ylim = c(45, 70)) +
  geom_point(aes(x = lon, y = lat, colour = colony), size = 10, alpha = 0.7) +
  geom_polygon(data = land_df_wgs,
               aes(x = long,
                   y = lat,
                   group = group),
...
uk_plot <-
  ggplot(meta_df %>% filter(area %in% c("UK"))) +
  coord_map(projection = "azequidistant",
            xlim = c(-20, 10),
            ylim = c(45, 65)) +
  geom_point(aes(x = lon, y = lat, colour = colony), size = 10, alpha = 0.7) +
...
norway_plot <- 
  ggplot(meta_df %>% filter(area %in% c("Norwegian Sea"))) +
  coord_map(projection = "azequidistant",
            xlim = c(-40, 30),
            ylim = c(60, 85)) +
  geom_point(aes(x = lon, y = lat, colour = colony), size = 10, alpha = 0.7) +
...
barents_plot <- 
  ggplot(meta_df %>% filter(area %in% c("Barents Sea"))) +
  coord_map(projection = "azequidistant",
            xlim = c(-10, 55),
            ylim = c(65, 85)) +
  geom_point(aes(x = lon, y = lat, colour = colony), size = 10, alpha = 0.7) +
...
space_plot <- plot_grid(atlantic_plot,
          plot_grid(iceland_plot, uk_plot, nrow = 1),
                    plot_grid(norway_plot, barents_plot, nrow = 1),
          col_plot,
          nrow = 4, rel_heights = c(0.9, 1, 1, 1.1))
