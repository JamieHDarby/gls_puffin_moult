...
for(i in 1:length(act.ls3)){
  ...
  df$flying <- ifelse(df$std_conductivity < 0.1, 1, 0)
  ...
  }else{moult.ls2[[i]] <- NULL; indicator[i] <- "too few daylight fixes"}
...
moult_df2 <- do.call(rbind, moult.ls2) %>%
  ...
  mutate(bout_id = ifelse(moult > 0,
    ...
    flight_simp = flight,
    rest = 1 - flight,
    rm_flight_simp = rm_flight)

# Append colony variable to moult dataframe
{
  moult_df2$area <- "Barents Sea"
  moult_df2$area[which(moult_df2$colony == "Skellig Michael")] <- "Ireland"
...
save(moult_df2, file = "data/cleaned/moult_df_act.RData")
save(moult.ls2, file = "data/cleaned/moult_ls_act.RData")
