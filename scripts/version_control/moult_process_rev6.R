...
for(i in 1:length(act.ls3)){
  ...
  if(class(df) == "data.frame"){
    ...
    for(k in 1:50){
      ...
      if(mean(ars.prop) > 0.15 & mean(ars.prop) < 0.3){
        ind.prop <- k/100
  ...
  df$rest <- ifelse((df$act > (1 - ind.prop)), 1, 0)
  df$flying <- ifelse((df$act < ind.prop), 1, 0)
  # df$rest <- ifelse((df$light.min < 0) & (df$act < 0.02), 1, df$rest)
  # Infer flying behaviour
  df$ars <- ifelse(
    (df$act > ind.prop) & (df$act < (1 - ind.prop)), 1, 0)
