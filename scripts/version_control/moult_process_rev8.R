...
act.ls3 <- act.df %>%
  ...
  .[!as.integer(format(.$date, "%j")) %in% 110:190, ] %>%
  # Split by id year
  split(., .$id_year) %>%
  discard(is.null) %>%
  .[sapply(., nrow) > 10000] %>%
...
for(i in 1:length(act.ls3)){#length(act.ls3)){
  ...
  df$long_fl <- 0
  for(k in 3:(nrow(df) - 2)){
    df$long_fl[k] <- ifelse((sum(df$act_ad[(k - 2):(k + 2)] < 0.03) == 5), 1, 0)
  }
  
  df$tuck <- (df$std_light_ad < df$light.min)
  ...
  days.df <- df %>%
    ...
    lapply(., function(x) {
      ...
      tuck <- (sum(x$tuck) / nrow(x))
      # sum wet
      # sum.wet <- (sum(x$act) / nrow(x))
      sum.wet <- (sum(x$act_ad) / nrow(x))
      flight_alt <- (1 - sum.wet)
      
      long_fl <- (sum(x$long_fl) / nrow(x))
      flight <- (sum(x$long_fl) / nrow(x))
      ...
      out <- data.frame(id, fixes, colony, col_dist, col_lat, col_lon,
                        ...
                        lat_eq, lat, lon, logger, ars, tuck, long_fl, flight_alt)
    }) %>%
  ...
  days.df <- days.df[!as.integer(format(days.df$date, "%j")) %in% 110:190, ]
...
if(median(moult.ls3[[i]]$rm_flight[which(moult.ls3[[i]]$prim_moult > 0)]) > 
   ((median(moult.ls3[[i]]$rm_flight)) * (3.5 / 5))){
  # moult.ls3[[i]] <- NULL
  j <- j + 1
