...
for(i in 1:length(combo.ls3)){
  ...
  df$flying <- ifelse(df$act < 0.1, 1, 0)
  ...
  days.df <- df %>%
    ...
    lapply(., function(x) {
      # Wet data per day
      y <- x[which(x$std_light > 0.1),]
      # Flying data per day
      flight <- (sum(y$flying == 1) / nrow(y))
      flight_simp <- (sum(x$flying == 1) / nrow(x))
      # Date
      if(is.na(flight)){flight <- 0; data <- "no"}else{data <- "yes"}
      date <- x$date[1]
      julian <- x$julian[1]
      year <- x$year[1]
      id_year <- x$id_year[1]
      ...
      out <- data.frame(id, fixes, colony, col_dist, col_lat, col_lon,
                        sex, date, flight, date, julian, year, id_year,
                        lat_eq, lat, lon, logger, flight_simp)
    }) %>%
  ...
  }else{moult.ls3[[i]] <- NULL; indicator[i] <- "too few daylight fixes"}
  ...
  if(max(days.df$moult) > 60 & indicator[i] == "fine"){
    moult.ls3[[i]] <- NULL; indicator[i] <- "failed method"}
  if(sum(days.df$rm_flight > 0.5) > 30 & indicator[i] == "fine"){
    moult.ls3[[i]] <- NULL;  indicator[i] <- "failed gls"}
  if(sum(days.df$moult_bool) > 100 & indicator[i] == "fine"){
    moult.ls3[[i]] <- NULL;  indicator[i] <- "failed method"}
  if(sum(days.df$fixes < 10) > 1 & indicator[i] == "fine"){
    moult.ls3[[i]] <- NULL;  indicator[i] <- "too few daylight fixes"}
  if(max(days.df$prim_moult) < 21 & indicator[i] == "fine"){
    moult.ls3[[i]] <- NULL;  indicator[i] <- "moult too short"}
...
# Append colony variable to moult dataframe
{
  ...
  moult_df3$area[which(moult_df3$colony == "Skellig Michael")] <- "Ireland"
