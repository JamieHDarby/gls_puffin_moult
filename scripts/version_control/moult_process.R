...
combo.ls3 <- combo.ls2 %>%
  ...
  .[!as.integer(format(date, "%j")) %in% 110:244, ] %>%
  # Split by id year
  split(., .$id_year)
...
for(i in 1:length(combo.ls3)){
  ...
  }else{moult.ls[[i]] <- NULL; indicator[i] <- "too few daylight fixes"}
  }else{moult.ls[[i]] <- NULL; indicator[i] <- "too few daylight fixes"}
  ...
  if(sum(days.df$fixes < 10) > 5 & indicator[i] == "fine"){
    moult.ls[[i]] <- NULL;  indicator[i] <- "too few daylight fixes"}
