...
lgh_df$leg_tuck <- ifelse(lgh_df$light_diff > 0.6, 1, 0)
...
ggplot(act_example) +
  ...
  geom_tile(aes(x = date, y = time, fill = std_conductivity_ad), height = 1000)

# Plot raw activity data timeseries
ggplot(act_example) +
  scale_fill_viridis_c(option = "A", trans = "reverse") +
  geom_tile(aes(x = date, y = time, fill = std_conductivity), height = 1000)

# Plot leg tucks identified
ggplot(act_example) +
  scale_fill_viridis_c(option = "A") +
  geom_tile(aes(x = date, y = time, fill = leg_tuck), height = 1000)
